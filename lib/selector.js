...
function test(node, index) {
  ...
  //console.error('test node %s', node.type)
  //console.error('test node %s', this.tag)

  // check for combinator only
  ...
  if(res && this.pseudo) {
    ...
    // html block and inline pseudo extensions
    }else if(
      ...
      //console.error('%s: %s', node.type, node._htmlBlockType)
  ...
  if(res && this.selector) {
    //if(this.tag === UNIVERSAL) {
      ////console.error('got universal selector on child selector');
      //nodes = this.children(node, this.selector, true);
      //res = nodes.length > 0;
