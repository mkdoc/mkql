...
function test(node, index) {
  ...
  // adjacent selector and no next node - can't possibly match
  if((this.adjacent || this.sibling) && !node.next) {
    return false; 
  }
  ...
  if(res && this.pseudo) {
    if(this.pseudo.literal === FIRST_CHILD) {
      ...
      //console.error('match on last child %s', node.parent)
      //console.error('match on last child %s', node.parent.lastChild === node)
