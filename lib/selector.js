...
function children(node, selector) {
  ...
  if(this.descendant) {
    ...
    while((event = walker.next()) !== null) {
      if(event.entering) {
        matcher = selector.test(event.node, i);
        if(matcher.matched) {
          nodes = nodes.concat(matcher.nodes);
        }
        i++;
...
function test(node, index) {
  ...
  if(res && this.pseudo) {
    ...
    // :nth-child
      ...
      var expects = this.nth(index);
      res = res && (index === expects);
    }
  }
...
function nth(index) {
  var val = this.pseudo.nth.digit1 * index
  if(this.pseudo.op2 === '+') {
    val = val + this.pseudo.digit2; 
  }else if(this.pseudo.op2 === '-') {
    val = val - this.pseudo.digit2; 
  }
  return val;
}
