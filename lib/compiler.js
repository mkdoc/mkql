var ast = require('mkast')
  ...
  , tokenizer = new RegExp(
      ...
      + '(' + attrs.source + ')*'
      // pseudo selectors
      + '(:(?:first-child|last-child|empty|(?:nth-child|not)\\(([^\\)]+)\\)))*'
...
function entry(match, parent) {
  ...
  if(match[5]) {
    ...
    if(match[6]) {
      ...
      }else if(o.pseudo.name === ':not') {

        // compile sub-selector
        var result = compiler(match[6]);

        // :not() only supports simple selectors
        if(result.selectors && result.selectors.length) {
          if(result.selectors.length > 1) {
            throw new Error('multiple selectors disallowed on :not()'); 
          }
          o.not = result.selectors[0];
        }
      }
    }
  }
  ...
  if(o.next) {
    ...
    // descendant selector: whitespace only
    o.child = /^\s+$/.test(o.next);
